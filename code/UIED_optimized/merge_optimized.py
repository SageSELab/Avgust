...
def is_same_line(comp1, comp2):
    mid_y1 = (comp1.bbox.row_min + comp1.bbox.row_max) / 2
    midx_y2 = (comp2.bbox.row_min + comp2.bbox.row_max) / 2
    if abs(midx_y2 - mid_y1) < 5:
        return True
    return False
...
def merge_label_with_comp(compos):
    mark = np.zeros(len(compos))
    for i, c1 in enumerate(compos):
        if c1.category == 'Text':
            continue
        if i < (len(compos) - 1):
            if compos[i+1].category == 'Text' and is_same_line(c1, compos[i+1]):
                c1.element_merge(compos[i+1])
                mark[i+1] = 1
